<aside id="main-sidebar" class="w-[260px] h-screen fixed left-0 top-0 bg-white border-r border-gray-100 flex flex-col py-6 z-50 transition-transform duration-300 -translate-x-full lg:translate-x-0">
    ...
    <nav class="flex-1 px-4 space-y-1">
        ...
        @elseif (Auth::user() && in_array(Auth::user()->role, ['admin', 'pembimbing']))
            ...
            <a href="{{ route('admin.master-data') }}" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm {{ request()->routeIs('admin.master-data') ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-gray-600 hover:bg-gray-50' }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M21 9H3"/><path d="M9 21V9"/></svg>
